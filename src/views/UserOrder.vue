<template>
    <div>
        <div class="admin">
            <el-table :data="Horderlist" height="455" style="width: 100%">
            ...
            <el-table-column prop="contract.lessor" label="房东" align="center"></el-table-column>
            <el-table-column prop="house.hphone" label="联系方式" align="center"></el-table-column>
            ...
            <el-table-column label="状态" prop="status" align="center">
                <template v-slot="props">
                    <el-tag v-if="props.row.status==='已退租'" :type="'danger'">{{ props.row.status}}</el-tag>
                    <el-tag v-if="props.row.status==='在租'" :type="'success'">{{ props.row.status}}</el-tag>
                    <el-tag v-if="props.row.status==='待审核'" :type="'warning'">{{ props.row.status}}</el-tag>
                  </template>
            </el-table-column>
            <el-table-column prop="unpaid" label="需缴费(元)" align="center"></el-table-column>
            <el-table-column prop="total" label="缴费总额(元)" align="center"></el-table-column>
            <el-table-column label="交租" align="center" :min-width="100">
                <template v-slot="props">
                    <el-button v-if="props.row.unpaid!==0" type="success" size="mini" @click="jiaozu(props.row)" >交租</el-button>
                </template>
            </el-table-column>
            <el-table-column label="报修" align="center" :min-width="100">
                <template v-slot="props">
                    <el-button v-if="props.row.status==='在租'" type="warning" size="mini" @click="repair(props.row)" >报修</el-button>
                </template>
            </el-table-column>
            <el-table-column label="退租" align="center" :min-width="100">
                <template v-slot="props">
                    <el-popconfirm v-if="props.row.status==='在租'&&props.row.unpaid===0" title="你确定要退租吗?" @confirm="tuizu(props.row)">
                        <el-button type="danger" slot="reference" size="mini" >退租</el-button>
                    </el-popconfirm>
                </template>
            </el-table-column>
            </el-table>
        ...
        <div>
            <el-dialog title="报修单" :visible.sync="repairDialogVisible" width="50%" center>
                <el-form :model="onerepair">
                    <el-form-item label="门牌号">
                        <el-input v-model="onerepair.hnumber"></el-input>
                    </el-form-item>
                    <el-form-item label="问题描述">
                        <el-input v-model="onerepair.content" type="textarea" :rows="5"></el-input>
                    </el-form-item>
                </el-form>
...
export default{
    ...
    methods:{
        ...
        findBySearch(){
            request.get("/horder/getHorderByuserid",{
                params:{
        ...
        // 自己手动获取
        gettotal(){
            request.get("/horder/gettotal",{params:{userid:this.user.userid}}).then(res=>{
                if(res.code==='0'){
                    this.total=res.data
                }
            })
        ...
        jiaozu(row){
            this.onehorder=Object.assign({},row);
            delete this.onehorder.house;
            delete this.onehorder.user;
            delete this.onehorder.contract;
            request.post("/horder/jiaozu",this.onehorder).then(res=>{
                if(res.code==='0'){
                    this.$notify({
                        title: '成功',
                        message: '缴费成功！',
        ...
        tuizu(row){
            this.onehorder=Object.assign({},row);
            this.$set(this.onehorder,"status","待审核")
            delete this.onehorder.house;
            delete this.onehorder.user;
            delete this.onehorder.contract;
            request.post("/horder/tuizu",this.onehorder).then(res=>{
                if(res.code==='0'){
                    this.$notify.info({
                        title: '通知',
                        message: '退租申请已发送，请等待管理员审核！',
                        type: 'success'
                    });
                }
                this.findBySearch();
        ...
        repair(row){
            this.$set(this.onerepair,"userid",row.user.userid)
            this.$set(this.onerepair,"houseid",row.house.houseid)
            this.$set(this.onerepair,"status","待处理")
            this.repairDialogVisible=true
        },
        exit(){
            request.post("/repair/add",this.onerepair).then(res=>{
                if(res.code==='0'){
                    this.$notify({
                        title: '成功',
                        message: '报修单已生成，请等待处理！',
                        type: 'success'
                    });
                    this.$set(this.onerepair,"hnumber","")
                    this.$set(this.onerepair,"content","")
