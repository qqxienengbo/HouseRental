<template>
    <div>
        ...
        <div class="admin">
            ...
            <el-table-column label="退租审批" align="center" :min-width="100">
                <template v-slot="props">
                    <el-button  type="success" size="mini" @click="tongyi(props.row)">同意</el-button>
                    <el-button  type="danger" size="mini" @click="butongyi(props.row)">不同意</el-button>
                </template>
            </el-table-column>
            </el-table>
        </div>
...
export default{
    ...
    methods:{
        ...
        tongyi(row){
            ...
            this.uphouse=Object.assign({},row.house)
            this.$set(this.uphouse,"houserstatus","未租")
            console.log("uphouse=",this.uphouse)
            this.$set(this.onehorder,"status","已退租")
            delete this.onehorder.house;
            delete this.onehorder.user;
            delete this.onehorder.contract;
            //更新租房列表的状态为已退租
            request.post("/horder/update",this.onehorder).then(res=>{
                if(res.code==='0'){
                    this.$message({
                        message:"同意退租！",
                        type:'success'
            ...
            //更新房源状态为未租，重新放到货架上供租客选择
            request.post("/house/save",this.uphouse).then(res=>{

            })
        },
        butongyi(row){
            this.onehorder=Object.assign({},row);
            this.$set(this.onehorder,"status","在租")
            delete this.onehorder.house;
            delete this.onehorder.user;
            delete this.onehorder.contract;
            request.post("/horder/update",this.onehorder).then(res=>{
